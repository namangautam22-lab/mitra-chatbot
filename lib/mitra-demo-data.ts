...
// ---------------------------------------------------------------------------
// TRACKING DOCUMENT URLS
// Single source of truth — swap these when live courier links are available.
// ---------------------------------------------------------------------------
/** Used when the document has been delivered */
export const TRACKING_URL_DELIVERED =
  "https://drive.google.com/file/d/1m0ccNJ4flJFTBy7XpTtSi8nBoWcUo48m/view?usp=sharing"

/** Used when the document is in-transit / dispatched */
export const TRACKING_URL_IN_TRANSIT =
  "https://drive.google.com/file/d/1m0ccNJ4flJFTBy7XpTtSi8nBoWcUo48m/view?usp=sharing"
...
const INTENT_RESPONSES: Record<string, BotResponse> = {
  ...
  // Session: documentIssue_originalRcMissing_*
  // User msgs: "RC missing", "RC kab tak milegi", "RC ki photo mngwa dijiye",
  //            "I haven't received my RC", "I need RC urgent"
  ...
  // Session: documentIssue_interstateNOC_*
  // User msgs: "Not received noc", "State noc please help",
  //            "Not responding 15 days noc transfer"
  ...
  // Session: documentIssue_loanNocNotReceived_*
  // User msgs: "Closer letter", "Loan NOC not available", "Mail copy"
  ...
  // Session: documentIssue_rtoFormMissing_*
  // User msgs: "Mail not sent to RTO", "Forms not received"
  ...
  // Session: documentIssue_sellerIdMissing_*
  // User msgs: "Aadhar card", "Pan card", "Second address proof",
  //            "adhar card front side not available"
  ...
  // Session: documentIssue_clearanceCertificate_*
  ...
  // Session: documentIssue_insuranceNotReceived_*
  // User msgs: "Need insurance copy", "where is my insurance"
  ...
  share_digital_copy: {
    ...
    card: {
      ...
      awb: "33176410845199",
      courier: "Delhivery",
      url: TRACKING_URL_IN_TRANSIT,
      expiresAt: new Date(Date.now() + 30 * 60 * 1000).toISOString(),
    },
  },

  // ── LEGACY / COMPATIBILITY ───────────────────────────────────────────────
  // "I haven't received my RC" old chip — now routes to the same response as
  // original_rc_missing so both old and new entry points give consistent info.

  rc_not_received: {
    text: "The Original RC is still pending and hasn't been received by CARS24 from the seller yet.\n\nWe're actively following up. A dedicated runner is assigned to collect it as soon as possible.\n\nWould you like me to raise a support ticket?",
    options: [
      { label: "Yes — raise a ticket", intent: "confirm_ticket" },
      { label: "Not now", intent: "dismiss_ticket" },
    ],
...
//
// IMPORTANT: More specific patterns MUST come BEFORE generic ones so that
// e.g. "where is my insurance" hits the insurance handler, not the generic
// "where / track / status" handler.
// ---------------------------------------------------------------------------
const FREE_TEXT_RESPONSES: Array<{ pattern: RegExp; response: BotResponse }> = [
  // ── RC / Registration Certificate ────────────────────────────────────────
  {
    pattern:
      /\brc\b|registration.?cert|rc.?miss|rc.?nahi|rc.?kab|rc.?photo|rc.?pic|rc.?copy|original.?rc|urgent.*rc|rc.*urgent|haven.?t.*receiv.*rc|not.*receiv.*rc|rc.*not.*receiv/i,
    response: INTENT_RESPONSES.original_rc_missing,
  },

  // ── Loan NOC (before general NOC) ────────────────────────────────────────
  {
    pattern: /loan.?noc|bank.?noc|closer.?letter|noc.*loan/i,
  ...
  // ── Insurance ─────────────────────────────────────────────────────────────
  // Placed BEFORE generic "where / track / status" so that
  // "where is my insurance" routes here instead of the generic document list.
  {
  ...
  // ── Generic tracking / status / "where" ──────────────────────────────────
  // LAST among document queries — specific patterns above take priority.
  {
...
// ---------------------------------------------------------------------------
// DEFAULT RESPONSE  (no pattern match)
// ---------------------------------------------------------------------------
